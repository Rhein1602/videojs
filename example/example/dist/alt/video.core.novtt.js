...
(function (global, factory) {
  ...
  var PlayNext = /*#__PURE__*/function (_Button) {
    inheritsLoose(PlayNext, _Button);
     ...
     *        The key/value store of player options.
     */
    function PlayNext(player, options) {
      var _this;
      ...
      options.replay = options.replay === undefined || options.replay; //绑定监听器
      // if (options.replay) {
    ...
    _proto.handleClick = function handleClick(event) {
      console_1.log("play-next click");
      console_1.log(this.player_.options_.sources);
      var the_src = this.player_.options_.sources[0];
      var len = this.player_.options_.sources.length; // 资源数组长度
      // 更新资源数组
    ...
    _proto.handleSeeked = function handleSeeked(event) {
      console_1.log("play-next seeked");
    }
    /**
     * Add the vjs-playing class to the element so it can change appearance.
     *
    ...
    _proto.handleEnded = function handleEnded(event) {
      console_1.log("play-next ended");
      console_1.log(this.player_.options_.sources);
      var the_src = this.player_.options_.sources[0];
      var len = this.player_.options_.sources.length;
  ...
  var AutoPlayNext = /*#__PURE__*/function (_Button) {
    inheritsLoose(AutoPlayNext, _Button);
     ...
     *        The key/value store of player options.
     */
    function AutoPlayNext(player, options) {
      var _this;
    ...
    var _proto = AutoPlayNext.prototype;
    ...
    _proto.handleClick = function handleClick(event) {
      console_1.log("auto-play-next click: " + flag);
  ...
  ControlBar.prototype.options_ = {
    children: ['playToggle', //播放按钮
    'volumePanel', //音量
    'currentTimeDisplay', //已播放时间
    'timeDivider', //分时器
    'durationDisplay', //总时间
    'progressControl', //进度条
    'liveDisplay', //剩余时间未知时使用，可能和直播相关
    'seekToLive', //剩余时间未知时使用，可能和直播相关
    'remainingTimeDisplay', //显示剩余时间
    'customControlSpacer', //客制化间隔，用于插入其他插件
    'playbackRateMenuButton', //倍数播放选项
    'chaptersButton', //章节切换（作用不明）
    'descriptionsButton', //切换描述（作用不明）
    'subsCapsButton', //字幕选择
    'audioTrackButton', //音轨按钮
    'fullscreenToggle', //全屏控件
  ...
  var TitleBar = /*#__PURE__*/function (_Component) {
    inheritsLoose(TitleBar, _Component);
    ...
    function TitleBar(player, options) {
      ...
      _this = _Component.call(this, player, options) || this; // It is important to invoke the superclass before anything else,
      ...
      if (options.text) {
        _this.updateTextContent(options.text);
      }
    ...
    _proto.updateTextContent = function updateTextContent(text) {
      console_1.log(text); // If no text was provided, default to "Text Unknown"
  ...
  TitleBar.prototype.controlText_ = 'title-bar'; // Register the component with Video.js, so it can be used in players.
  ...
  var Player = /*#__PURE__*/function (_Component) {
    ...
    _proto.userActive = function userActive(bool) {
      console_1.log('user active: ' + bool);
    ...
    _proto.listenForUserActivity_ = function listenForUserActivity_() {
      ...
      this.setInterval(function () {
        ...
        inactivityTimeout = this.setTimeout(function () {
          console_1.log('time out'); // Protect against the case where the inactivityTimeout can trigger just
          // before the next user activity is picked up by the activity check loop
          // causing a flicker

          if (!this.userActivity_) {
            console_1.log('inactive');
  ...
  function videojs$2(id, options, ready) {
    var player = videojs$2.getPlayer(id); //log.warn('this is a test by zzf');

    console.log('this is a test by zzf');
    ...
    options = options || {};
    videojs$2.hooks('beforesetup').forEach(function (hookFunction) {
    ...
    videojs$2.hooks('setup').forEach(function (hookFunction) {
  ...
  videojs$2.hooks = function (type, fn) {
    videojs$2.hooks_[type] = videojs$2.hooks_[type] || [];

    if (fn) {
      videojs$2.hooks_[type] = videojs$2.hooks_[type].concat(fn);
    }

    return videojs$2.hooks_[type];
  };
  /**
  ...
  videojs$2.hook = function (type, fn) {
    videojs$2.hooks(type, fn);
  };
  /**
  ...
  videojs$2.hookOnce = function (type, fn) {
    videojs$2.hooks(type, [].concat(fn).map(function (original) {
      var wrapper = function wrapper() {
        videojs$2.removeHook(type, wrapper);
  ...
  videojs$2.removeHook = function (type, fn) {
    var index = videojs$2.hooks(type).indexOf(fn);

    if (index <= -1) {
      return false;
    }

    videojs$2.hooks_[type] = videojs$2.hooks_[type].slice();
    videojs$2.hooks_[type].splice(index, 1);
    return true;
  }; // Add default styles
  ...
  videojs$2.getPlayer = function (id) {
    var players = Player.players;
    var tag;
  ...
  videojs$2.getAllPlayers = function () {
  ...
  videojs$2.getComponent = Component.getComponent;
  /**
  ...
  videojs$2.registerComponent = function (name, comp) {
    if (Tech.isTech(comp)) {
  ...
  videojs$2.TOUCH_ENABLED = TOUCH_ENABLED;
  videojs$2.extend = extend;
  videojs$2.mergeOptions = mergeOptions;
  videojs$2.bind = bind;
  ...
  videojs$2.plugin = function (name, plugin) {
  ...
  videojs$2.addLanguage = function (code, data) {
    ...
    videojs$2.options.languages = mergeOptions(videojs$2.options.languages, (_mergeOptions = {}, _mergeOptions[code] = data, _mergeOptions));
  ...
  videojs$2.log = log;
  videojs$2.createLogger = createLogger$1;
  videojs$2.createTimeRange = videojs$2.createTimeRanges = createTimeRanges;
  videojs$2.formatTime = formatTime;
  videojs$2.setFormatTime = setFormatTime;
  videojs$2.resetFormatTime = resetFormatTime;
  videojs$2.parseUrl = parseUrl;
  videojs$2.isCrossOrigin = isCrossOrigin;
  videojs$2.EventTarget = EventTarget;
  videojs$2.on = on;
  videojs$2.one = one;
  videojs$2.off = off;
  ...
  videojs$2.computedStyle = computedStyle;
  /**
  ...
  videojs$2.url = Url;
  videojs$2.defineLazyProperty = defineLazyProperty;
